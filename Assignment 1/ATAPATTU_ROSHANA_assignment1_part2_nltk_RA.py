...
Original file is located at
    https://colab.research.google.com/drive/1i3kDq2VR0zhAHL4Bv7Xx7YqcQnxOa-7z
...
def question_two():
    ...
    total_tokens = len(moby_tokens)
    whale_count = sum(1 for token in moby_tokens if token.lower() == 'whale')
...
def question_five():
    ...
    # YOUR CODE HERE
     ...
     # Get only alphabetic tokens
    alphabetic_tokens = [token for token in moby_tokens if token.isalpha()]

    if not alphabetic_tokens:
        return ("", 0)

    longest_word = max(alphabetic_tokens, key=len)
...
def question_six():
    ...
    for word, freq in freq_dist.items():
        if freq > 2000 and word.isalpha():
            high_freq_words.append((freq, word))
...
def question_seven():
    ...
    if len(sentences) > 0:
        average = total_tokens / len(sentences)
    else:
        average = 0
...
def question_eight():
    ...
    # Filter tokens: only alphabetic and not stop words
    filtered_tokens = [
...
def question_nine():
    ...
    # Get alphabetic tokens and stem them
    alphabetic_tokens = [token.lower() for token in moby_tokens if token.isalpha()]
    stems = [stemmer.stem(token) for token in alphabetic_tokens]
